...
export default function Profile({ profile, update, setGrade, onChangeMode, onEditTimetable, onReset }) {
      ...
      <div className="p-menu">
        ...
        {editing ? (
          ...
          <>
            ...
            <button type="button" className="p-row" onClick={() => { setCredits(profile.credits); setEditing(true) }}>
              <span>학점 직접 수정</span>
        ...
        <div className="p-note" style={{ paddingTop: 4 }}>학년을 바꾸면 학점·시간표가 그 학년 표준 샘플로 다시 깔려요. 대화도 새로 시작돼요.</div>
        ...
        <div className="p-note">
          입력한 정보는 이 기기 브라우저에만 저장돼요. 동학은 국립순천대학교 인공지능공학부 학생이 만든 바이브코딩 경진대회 출품작이며, 교육과정·학사일정·공지는 샘플이에요. 답변은 참고용이고 최종 확인은 학과 사무실에서 해주세요.
        </div>
      </div>
    </div>
  )
